<template>
  <div>
          <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>导入项目管理</el-breadcrumb-item>
          </el-breadcrumb>
          <!--卡片视图 -->
          <el-card class="card">
            <!--搜索与添加区 -->
    ...
    <el-dialog
      ...
      <el-form ref="form" label-width="80px" >
        ...
        <el-form-item label="输入导入项目id*必填">
          <el-input v-model="addParams.itemId"></el-input>
        </el-form-item>
        <el-form-item label="输入该导入项目数值">
...
export default {
  ...
  methods: {
    getList () {
      const _this = this
      _this.$http.post('/api/salary/importItem/selectByCondition', this.searchParams).then(
        function (res1) {
          console.log('--------******--------')
    ...
    async loadCategories () {
      ...
      const _this = this
      const response = await _this.$http.post('/api/salary/importItem/selectByCondition', this.searchParams)
    ...
    addEnter () {
      ...
      const _this = this
      const response = _this.$http.post('/api/salary/importItem/addRecord', this.addParams)
      console.log(response)
      _this.getList()
    },
    Edit (id) {
    ...
    editEnter () {
      ...
      const _this = this
      const response = _this.$http.post('/api/salary/importItem/changeRecord', this.editParams)
      console.log(response)
      _this.getList()
    },
    Delete (id) {
      ...
      const _this = this
      _this.$http.post('/api/salary/importItem/deleteRecord', this.deleteParams).then(
        function (res1) {
