<template>
  <div>
          <el-breadcrumb separator-class="el-icon-arrow-right">
            ...
            <el-breadcrumb-item>工资结算</el-breadcrumb-item>
          </el-breadcrumb>
          <!--卡片视图 -->
          <el-card>
            <!--搜索与添加区 -->
            <el-row :gutter="20">
              ...
              <el-col :span="3"><el-button size="small" type="danger" @click="dialogVisible = true">新增结算</el-button></el-col>
            </el-row>
            ...
            <el-table :data="tableData">
              ...
              <el-table-column label="实发月份" prop="date">
              </el-table-column>
              <el-table-column label="实发" prop="value">
              </el-table-column>
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  ...
                  <el-button size="small" type="danger" @click="Send(scope.row.employeeId,scope.row.date)">发放</el-button>
                </template>
              </el-table-column>
            </el-table>
          </el-card>
...
export default {
  ...
  methods: {
    getList () {
      const _this = this
      _this.$http.post('/api/salary/actualSalaryItem/selectByCondition', this.searchParams).then(
        function (res1) {
          console.log('--------******--------')
    ...
    async loadCategories () {
      ...
      const _this = this
      const response = await _this.$http.post('/api/salary/actualSalaryItem/settleSalary', this.searchParams)
    ...
    enter () {
      ...
      const _this = this
      const response = _this.$http.post('/api/salary/actualSalaryItem/selectByCondition', this.addParams)
      console.log(response)
      _this.getList()
    },
    Delete (id) {
      ...
      const _this = this
      _this.$http.post('/api/salary/actualSalaryItem/deleteRecord', this.deleteParams).then(
        function (res1) {
    ...
    Send (employeeId, date) {
      this.updataParams.date = date
      this.updataParams.employeeId = employeeId
      const _this = this
      _this.$http.post('/api/salary/actualSalaryItem/paySalary', this.updataParams)
      _this.getList()
    }
  }
}
